...
#include "json.hpp"
#include "spline.h"              // Copyright (C) 2011, 2014 Tino Kluge (ttk448 at gmail.com)
...
int main() {
  ...
  h.onDisconnection([&h](uWS::WebSocket<uWS::SERVER> ws, int code,
    ...
    int port = 4567;
    if (h.listen(port)) {
        cout << "Path Planning message handling start: listening on port=" << port << endl;
    } else {
        cout << "Path Planning message handler failed: listening on port=" << port << " Exiting." << endl;
        cerr << "Path Planning message handler failed: listening on port=" << port << " Exiting." << endl;
    return -1;
    }
